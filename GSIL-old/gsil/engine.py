...
class Engine(object):
    ...
    def search(self, rule_object):
        ...
        try:
            ...
            if self.rule_object.filename is not None:
                for filename in self.rule_object.filename.split(','):
                    name_query += 'filename:{name} '.format(name=filename.strip().lower())
            keyword = '{keyword} {ext} {name}'.format(keyword=self.rule_object.keyword, ext=ext_query, name=name_query)
