...
class Rule(object):
    def __init__(self, types=None, corp=None, keyword=None, mode='normal-match', extension=None, filename=None):
        self.types = types
        self.corp = corp
            ...
            for corp_name, corp_rules in rule_list.items():
                for rule_keyword, rule_attr in corp_rules.items():
                    ...
                    if 'ext' in rule_attr:
                        ...
                        # 默认为空
                        extension = None
                    if 'filename' in rule_attr:
                        filename = rule_attr['filename'].strip()
                    else:
                        # 默认为空
                        filename = None
                    r = Rule(types, corp_name, rule_keyword, mode, extension, filename)
